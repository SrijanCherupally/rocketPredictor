  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {activeSection === 'settings' ? <SettingsPanel units={units} setUnits={changeUnits} targetAltitude={targetAltitude} setTargetAltitude={changeTargetAltitude} theme={theme} setTheme={setTheme} /> : activeSection === 'flights' ? <><div className="date-filter"><label>From <input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} /></label><label>To <input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} /></label><button className="text-button" onClick={() => { setDateFrom(''); setDateTo('') }}>Clear</button></div><FlightsPanel launches={launches.filter((launch) => (!dateFrom || launch.date >= dateFrom) && (!dateTo || launch.date <= dateTo))} displayAltitude={displayAltitude} displayMass={displayMass} displayWind={displayWind} displayTemperature={displayTemperature} exportData={exportData} importData={importData} onDelete={removeLaunch} onEdit={editLaunch} onNew={openNewLaunch} /></> : <>
