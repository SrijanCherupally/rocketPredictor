...
import { AlertTriangle, ArrowDownToLine, ArrowUpRight, BarChart3, Bell, Check, ChevronDown, CloudSun, Database, Download, Gauge, Lightbulb, LogOut, Menu, Moon, Pencil, Plus, Rocket, Settings2, Sparkles, Sun, Target, Trash2, Wind, X, Activity, Upload } from 'lucide-react'
...
// TARC altitude scoring is measured in whole feet. "Over points" isolates the
// portion of that score caused by flying above the configured altitude goal.
const tarcOverPoints = (altitude: number, targetAltitude: number) => Math.max(0, Math.round(altitude) - Math.round(targetAltitude))
...
function App() {
  ...
  const [simulatorWeather, setSimulatorWeather] = useState<WeatherReference>({ wind: 4, pressure: 29.92, humidity: 50, temperature: DEFAULT_TEMPERATURE })
  ...
  const [activeSection, setActiveSection] = useState<'overview' | 'flights' | 'insights' | 'settings'>('overview')
  ...
  const descentModel = useMemo(() => descentRegression(launches), [launches])
  ...
  const reference = useMemo(() => ({ wind: median(graphLaunches.map((launch) => launch.windSpeed)), pressure: median(graphLaunches.map((launch) => launch.airPressure)), humidity: median(graphLaunches.map((launch) => launch.humidity)), temperature: median(graphLaunches.map((launch) => launch.temperature)) }), [graphLaunches])
  ...
  const descentSensitivity = useMemo(() => {
    const minMass = Math.max(350, Math.floor((graphBounds.minMass || 500) / 25) * 25)
    const maxMass = Math.min(900, Math.ceil((graphBounds.maxMass || 700) / 25) * 25)
    const massSpan = Math.max(1, maxMass - minMass)
    return Array.from({ length: 9 }, (_, index) => {
      const mass = minMass + (massSpan * index) / 8
      const predict = (altitude: number) => descentModel ? predictDescentTime(descentModel, { ...descentConditions, mass, altitude }) : 0
      return { mass, lowAltitude: predict(descentConditions.altitude * 0.9), targetAltitude: predict(descentConditions.altitude), highAltitude: predict(descentConditions.altitude * 1.1) }
    })
  }, [graphBounds, descentConditions, descentModel])
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <section className="graph-filters"><div><span>GRAPH FILTERS</span><strong>Explore your launch envelope</strong></div><label className="date-filter"> <span className="date-filter-heading"><span>Launch date</span><b>{new Date(selectedDateWindow[0]).toLocaleDateString()} – {new Date(selectedDateWindow[1]).toLocaleDateString()}</b></span><span className="date-slider"><span className="date-slider-track" /><span className="date-slider-fill" style={{ left: `${((selectedDateWindow[0] - graphBounds.minDate) / Math.max(1, graphBounds.maxDate - graphBounds.minDate)) * 100}%`, right: `${100 - ((selectedDateWindow[1] - graphBounds.minDate) / Math.max(1, graphBounds.maxDate - graphBounds.minDate)) * 100}%` }} /><input aria-label="Start launch date" type="range" min={graphBounds.minDate} max={graphBounds.maxDate} value={selectedDateWindow[0]} onChange={(event) => setDateWindow([Math.min(Number(event.target.value), selectedDateWindow[1]), selectedDateWindow[1]])} /><input aria-label="End launch date" type="range" min={graphBounds.minDate} max={graphBounds.maxDate} value={selectedDateWindow[1]} onChange={(event) => setDateWindow([selectedDateWindow[0], Math.max(Number(event.target.value), selectedDateWindow[0])])} /></span></label><button className="text-button" onClick={() => { setMassWindow(null); setDateWindow(null) }}>Reset</button></section>
        ...
        <section className="stats-grid"><StatCard label="FLIGHTS LOGGED" value={String(launches.length).padStart(2, '0')} note="+2 this month" trend="up" icon={<Database size={18} />} /><StatCard label="AVG. ALTITUDE" value={formatNumber(avgAltitude)} unit="ft" note={`${targetGap >= 0 ? '+' : ''}${formatNumber(targetGap)} ft vs target`} trend={targetGap >= 0 ? 'up' : 'down'} icon={<ArrowUpRight size={18} />} /><StatCard label="AVG. DESCENT" value={formatNumber(avgDescent, 1)} unit="sec" note="Target: 34.0 sec" trend={Math.abs(avgDescent - 34) < 2 ? 'up' : 'down'} icon={<ArrowDownToLine size={18} />} /><StatCard label="MODEL CONFIDENCE" value={adjustedModel ? formatNumber(adjustedModel.r2 * 100) : '—'} unit={adjustedModel ? '%' : ''} note={adjustedModel ? 'Weather model active' : 'Need 4+ flights'} trend="up" icon={<Gauge size={18} />} /></section>
        <section className="analysis-grid"><AnalysisCard title="Raw altitude model" subtitle="Altitude vs. total mass · no weather compensation" icon={<Activity size={18} />} accent="blue" model={rawModel} recommendation={rawRecommendation} chart={<RawChart data={rawChart} target={targetAltitude} units={units} />} /><AnalysisCard title="Adjusted altitude model" subtitle="Compensated for wind, pressure, humidity & temperature" icon={<CloudSun size={18} />} accent="purple" model={adjustedModel} recommendation={adjustedRecommendation} chart={<AdjustedChart data={adjustedChart} target={targetAltitude} units={units} />} /></section>
        <section className="descent-card analysis-card"><div className="card-heading"><div><div className="card-title"><span className="analysis-icon blue"><ArrowDownToLine size={18} /></span><h2>Descent time sensitivity</h2></div><p>Live model: weight and altitude effects at the entered launch conditions</p></div></div><div className="chart-wrap">{descentModel ? <DescentSensitivityChart data={descentSensitivity} /> : <div className="empty-chart"><Sparkles size={25} /><b>Keep logging flights</b><span>We need at least 4 complete flights to learn descent time.</span></div>}</div><div className="descent-legend"><span><i className="legend-dot low" />90% altitude</span><span><i className="legend-dot target" />Entered altitude</span><span><i className="legend-dot high" />110% altitude</span><b>{descentModel ? `Typical error: ±${formatNumber(descentModel.mae, 1)} sec` : 'Learning from flights'}</b></div></section>
...
function DescentSensitivityChart({ data }: { data: Array<{ mass: number; lowAltitude: number; targetAltitude: number; highAltitude: number }> }) { const values = data.flatMap((point) => [point.lowAltitude, point.targetAltitude, point.highAltitude]); const minimum = Math.floor(Math.min(...values) - 1); const maximum = Math.ceil(Math.max(...values) + 1); return <ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{ top: 14, right: 18, bottom: 4, left: -10 }}><CartesianGrid vertical={false} stroke="#e9edf3" /><XAxis dataKey="mass" type="number" tickLine={false} axisLine={false} tick={{ fill: '#8791a4', fontSize: 11 }} tickFormatter={(value) => `${formatMass(Number(value))}g`} /><YAxis domain={[minimum, maximum]} tickLine={false} axisLine={false} tick={{ fill: '#8791a4', fontSize: 11 }} tickFormatter={(value) => `${value}s`} /><Tooltip formatter={(value) => [`${Number(value).toFixed(1)} sec`, 'Descent time']} labelFormatter={(value) => `${formatMass(Number(value))}g`} /><Line name="90% altitude" dataKey="lowAltitude" stroke="#3478f6" strokeWidth={2.5} dot={false} /><Line name="Entered altitude" dataKey="targetAltitude" stroke="#7758d8" strokeWidth={2.5} dot={false} /><Line name="110% altitude" dataKey="highAltitude" stroke="#dc8d63" strokeWidth={2.5} dot={false} /></ComposedChart></ResponsiveContainer> }

function MassSimulator({ units, model, fallbackModel, targetAltitude, weather, setWeather, optimalMass, predictedAltitude, observedMasses }: { units: Units; model: ReturnType<typeof adjustedRegression>; fallbackModel: ReturnType<typeof linearRegression>; targetAltitude: number; weather: WeatherReference; setWeather: React.Dispatch<React.SetStateAction<WeatherReference>>; optimalMass: number | null; predictedAltitude: number | null; observedMasses: number[] }) {
  const update = (field: keyof WeatherReference, value: string) => setWeather((current) => ({ ...current, [field]: Number(value) }))
  ...
  const pressureValue = units === 'imperial' ? weather.pressure : weather.pressure * 33.8639
  const temperatureValue = units === 'imperial' ? weather.temperature : fahrenheitToCelsius(weather.temperature)
  return <section className="mass-simulator"><div className="simulator-heading"><div><p className="eyebrow">FLIGHT CONFIGURATION</p><h2>Rocket mass simulator</h2><p>Find the mass that should reach your target altitude in the conditions you enter.</p></div><div className="simulator-badge">{model ? 'WEATHER MODEL' : fallbackModel ? 'RAW MODEL' : 'NEEDS 3 FLIGHTS'}</div></div><div className="simulator-body"><div className="simulator-inputs"><label>WIND <div className="simulator-input"><input type="number" min="0" step="0.1" value={weather.wind * (units === 'imperial' ? 1 : 1.60934)} onChange={(event) => update('wind', String(Number(event.target.value) / (units === 'imperial' ? 1 : 1.60934)))} /><span>{units === 'imperial' ? 'mph' : 'km/h'}</span></div></label><label>TEMPERATURE <div className="simulator-input"><input type="number" step="0.1" value={temperatureValue} onChange={(event) => update('temperature', String(units === 'imperial' ? Number(event.target.value) : celsiusToFahrenheit(Number(event.target.value))))} /><span>{units === 'imperial' ? '°F' : '°C'}</span></div></label><label>AIR PRESSURE <div className="simulator-input"><input type="number" min="0" step="0.01" value={pressureValue} onChange={(event) => update('pressure', String(units === 'imperial' ? Number(event.target.value) : Number(event.target.value) / 33.8639))} /><span>{units === 'imperial' ? 'inHg' : 'hPa'}</span></div></label><label>HUMIDITY <div className="simulator-input"><input type="number" min="0" max="100" step="1" value={weather.humidity} onChange={(event) => update('humidity', event.target.value)} /><span>%</span></div></label></div><div className="simulator-result"><span className="footer-label">OPTIMAL ROCKET MASS</span><strong>{optimalMass !== null && optimalMass > 0 ? `${formatRecommendationMass(optimalMass)} g` : 'Collect more data'}</strong><span>{predictedAltitude !== null ? `Predicted ${formatNumber(predictedAltitude)} ft at target conditions` : model || fallbackModel ? 'Model needs a meaningful mass trend' : 'Log at least 3 flights to simulate'}</span>{outsideRange && <small>Recommendation is outside your logged mass range ({formatMass(minObserved)}–{formatMass(maxObserved)} g).</small>}</div></div></section>
}

function DescentSimulator({ units, model, conditions, setConditions, predictedDescent }: { units: Units; model: ReturnType<typeof descentRegression>; conditions: DescentConditions; setConditions: React.Dispatch<React.SetStateAction<DescentConditions>>; predictedDescent: number | null }) {
  const update = (field: keyof DescentConditions, value: string) => setConditions((current) => ({ ...current, [field]: Number(value) }))
  const pressure = units === 'imperial' ? conditions.pressure : conditions.pressure * 33.8639
  const temperature = units === 'imperial' ? conditions.temperature : fahrenheitToCelsius(conditions.temperature)
  return <section className="mass-simulator descent-simulator"><div className="simulator-heading"><div><p className="eyebrow">DESCENT CONFIGURATION</p><h2>Descent time predictor</h2><p>Predict descent time from the complete flight profile. The model retrains as you add flights.</p></div><div className="simulator-badge">{model ? `${model.sampleSize} FLIGHT MODEL` : 'NEEDS 4 FLIGHTS'}</div></div><div className="simulator-body"><div className="simulator-inputs descent-inputs"><label>MASS <div className="simulator-input"><input type="number" min="1" step="1" value={conditions.mass} onChange={(event) => update('mass', event.target.value)} /><span>g</span></div></label><label>ALTITUDE <div className="simulator-input"><input type="number" min="1" step="1" value={conditions.altitude} onChange={(event) => update('altitude', event.target.value)} /><span>ft</span></div></label><label>PARACHUTE <div className="simulator-input"><input type="number" min="1" step="0.1" value={conditions.parachuteSize} onChange={(event) => update('parachuteSize', event.target.value)} /><span>in</span></div></label><label>FLIGHT TIME <div className="simulator-input"><input type="number" min="0" step="0.1" value={conditions.flightTime} onChange={(event) => update('flightTime', event.target.value)} /><span>sec</span></div></label><label>WIND <div className="simulator-input"><input type="number" min="0" step="0.1" value={conditions.wind * (units === 'imperial' ? 1 : 1.60934)} onChange={(event) => update('wind', String(Number(event.target.value) / (units === 'imperial' ? 1 : 1.60934)))} /><span>{units === 'imperial' ? 'mph' : 'km/h'}</span></div></label><label>TEMPERATURE <div className="simulator-input"><input type="number" step="0.1" value={temperature} onChange={(event) => update('temperature', String(units === 'imperial' ? Number(event.target.value) : celsiusToFahrenheit(Number(event.target.value))))} /><span>{units === 'imperial' ? '°F' : '°C'}</span></div></label><label>AIR PRESSURE <div className="simulator-input"><input type="number" min="0" step="0.01" value={pressure} onChange={(event) => update('pressure', String(units === 'imperial' ? Number(event.target.value) : Number(event.target.value) / 33.8639))} /><span>{units === 'imperial' ? 'inHg' : 'hPa'}</span></div></label><label>HUMIDITY <div className="simulator-input"><input type="number" min="0" max="100" step="1" value={conditions.humidity} onChange={(event) => update('humidity', event.target.value)} /><span>%</span></div></label></div><div className="simulator-result"><span className="footer-label">PREDICTED DESCENT TIME</span><strong>{predictedDescent !== null ? `${formatNumber(predictedDescent, 1)} sec` : 'Collect more data'}</strong><span>{model ? `Expected accuracy: ±${formatNumber(model.mae, 1)} sec` : 'Log 4 complete flights to activate predictions'}</span></div></div></section>
}

function DescentPredictor({ units, model, conditions, setConditions, predictedDescent }: { units: Units; model: ReturnType<typeof descentRegression>; conditions: DescentConditions; setConditions: React.Dispatch<React.SetStateAction<DescentConditions>>; predictedDescent: number | null }) {
  const update = (field: keyof DescentConditions, value: number) => setConditions((current) => ({ ...current, [field]: value }))
  const fields: Array<{ label: string; field: keyof DescentConditions; value: number; unit: string; min?: number; step?: number; convert?: (value: number) => number }> = [
    { label: 'MASS', field: 'mass', value: conditions.mass, unit: 'g', min: 1 },
    { label: 'ALTITUDE', field: 'altitude', value: conditions.altitude, unit: 'ft', min: 1 },
    { label: 'PARACHUTE', field: 'parachuteSize', value: conditions.parachuteSize, unit: 'in', min: 1, step: 0.1 },
    { label: 'WIND', field: 'wind', value: conditions.wind * (units === 'imperial' ? 1 : 1.60934), unit: units === 'imperial' ? 'mph' : 'km/h', min: 0, step: 0.1, convert: (value) => value / (units === 'imperial' ? 1 : 1.60934) },
    { label: 'TEMPERATURE', field: 'temperature', value: units === 'imperial' ? conditions.temperature : fahrenheitToCelsius(conditions.temperature), unit: units === 'imperial' ? '°F' : '°C', step: 0.1, convert: (value) => units === 'imperial' ? value : celsiusToFahrenheit(value) },
    { label: 'AIR PRESSURE', field: 'pressure', value: units === 'imperial' ? conditions.pressure : conditions.pressure * 33.8639, unit: units === 'imperial' ? 'inHg' : 'hPa', min: 0, step: 0.01, convert: (value) => units === 'imperial' ? value : value / 33.8639 },
    { label: 'HUMIDITY', field: 'humidity', value: conditions.humidity, unit: '%', min: 0, step: 1 },
  ]
  return <section className="mass-simulator descent-simulator"><div className="simulator-heading"><div><p className="eyebrow">DESCENT CONFIGURATION</p><h2>Descent time predictor</h2><p>Predict descent time before launch from configuration and weather. The model retrains as flights are added.</p></div><div className="simulator-badge">{model ? `${model.sampleSize} FLIGHT MODEL` : 'NEEDS 4 FLIGHTS'}</div></div><div className="simulator-body"><div className="simulator-inputs descent-inputs">{fields.map((input) => <label key={input.field}>{input.label}<div className="simulator-input"><input type="number" min={input.min} step={input.step ?? 1} value={input.value} onChange={(event) => { const value = Number(event.target.value); update(input.field, input.convert ? input.convert(value) : value) }} /><span>{input.unit}</span></div></label>)}</div><div className="simulator-result"><span className="footer-label">PREDICTED DESCENT TIME</span><strong>{predictedDescent !== null ? `${formatNumber(predictedDescent, 1)} sec` : 'Collect more data'}</strong><span>{model ? `Expected accuracy: ±${formatNumber(model.mae, 1)} sec` : 'Log 4 complete flights to activate predictions'}</span></div></div></section>
}
...
function AltitudePredictorWithConfidence({ units, model, fallbackModel, targetAltitude, weather, setWeather, optimalMass, predictedAltitude, observedMasses, confidence, weights }: { units: Units; model: ReturnType<typeof adjustedRegression>; fallbackModel: ReturnType<typeof linearRegression>; targetAltitude: number; weather: WeatherReference; setWeather: React.Dispatch<React.SetStateAction<WeatherReference>>; optimalMass: number | null; predictedAltitude: number | null; observedMasses: number[]; confidence: number[]; weights: number[] }) {
...
function DescentPredictorWithConfidence({ units, model, conditions, setConditions, predictedDescent, confidence, weights }: { units: Units; model: ReturnType<typeof descentRegression>; conditions: DescentConditions; setConditions: React.Dispatch<React.SetStateAction<DescentConditions>>; predictedDescent: number | null; confidence: number[]; weights: number[] }) {
  ...
  return <section className="mass-simulator descent-simulator"><div className="simulator-heading"><div><p className="eyebrow">DESCENT CONFIGURATION</p><h2>Descent time predictor</h2><p>Predict descent time before launch from configuration and weather. The model retrains as flights are added.</p></div><div className="simulator-badge">{model ? `${model.sampleSize} FLIGHT MODEL` : 'NEEDS 4 FLIGHTS'}</div></div><div className="simulator-body"><div className="simulator-inputs descent-inputs">{fields.map((input) => <ImpactField key={input.field} label={input.label} confidence={input.confidence} weight={weightByField[input.field]}><div className="simulator-input"><input type="number" min={input.min} step={input.step ?? 1} value={input.value} onChange={(event) => { const value = Number(event.target.value); update(input.field, input.convert ? input.convert(value) : value) }} /><span>{input.unit}</span></div></ImpactField>)}</div><div className="simulator-result"><span className="footer-label">PREDICTED DESCENT TIME</span><strong>{predictedDescent !== null ? `${formatNumber(predictedDescent, 1)} sec` : 'Collect more data'}</strong><span>{model ? `Expected accuracy: ±${formatNumber(model.mae, 1)} sec` : 'Log 4 complete flights to activate predictions'}</span>{equation && <code className="prediction-equation">{equation}</code>}</div></div></section>
}
...
function FlightTable({ launches, targetAltitude, displayAltitude, displayMass, displayWind, displayTemperature, onDelete, onEdit }: { launches: Launch[]; targetAltitude: number; displayAltitude: (v: number) => string; displayMass: (v: number) => string; displayWind: (v: number) => string; displayTemperature: (v: number) => string; onDelete: (id: string) => void; onEdit: (launch: Launch) => void }) { return <div className="table-card"><div className="flight-table table-header"><span>DATE</span><span>ALTITUDE</span><span>TARC OVER</span><span>TOTAL MASS</span><span>WIND</span><span>TEMP</span><span>DESCENT</span><span>STATUS</span><span /></div>{launches.map((launch) => { const overPoints = tarcOverPoints(launch.altitude, targetAltitude); const near = Math.abs(launch.altitude - targetAltitude) < 20; return <div className="flight-table" key={launch.id}><span className="date-cell"><b>{new Date(`${launch.date}T12:00:00`).toLocaleDateString('en-US', { month: 'short', day: '2-digit' })}</b><small>{launch.id.replace('flight-', 'Flight ')}</small></span><strong>{displayAltitude(launch.altitude)}</strong><span className={overPoints ? 'tarc-over-points' : 'tarc-over-points zero'} title={`Points for ${overPoints ? 'flying above' : 'not exceeding'} the ${formatNumber(targetAltitude)} ft target`}>{overPoints} pts</span><span>{displayMass(totalMass(launch))}</span><span className="wind-cell"><Wind size={14} /> {displayWind(launch.windSpeed)}</span><span>{displayTemperature(launch.temperature)}</span><span>{formatNumber(launch.descentTime, 1)} sec</span><span className={`status ${near ? 'on-target' : 'review'}`}><i /> {near ? 'On target' : 'Review'}</span><span className="row-actions"><button className="edit-button" onClick={() => onEdit(launch)} aria-label={`Edit ${launch.id}`}><Pencil size={14} /></button><button className="delete-button" onClick={() => onDelete(launch.id)} aria-label={`Delete ${launch.id}`}><Trash2 size={15} /></button></span></div>})}</div> }
...
function TarcOverPointsInsight({ launches, targetAltitude }: { launches: Launch[]; targetAltitude: number }) {
  const scores = launches.map((launch) => tarcOverPoints(launch.altitude, targetAltitude))
  const average = scores.length ? scores.reduce((sum, score) => sum + score, 0) / scores.length : 0
  const flightsOverTarget = scores.filter((score) => score > 0).length
  const bestScore = scores.length ? Math.min(...scores) : null

  return <section className="insights-grid tarc-insights"><article className="insight-card"><div className="insight-heading"><Target size={18} /><div><h2>TARC over-points</h2></div></div>{launches.length ? <><p className="insight-copy">Your flights average <b>{formatNumber(average, 1)} over points</b> against the current {formatNumber(targetAltitude)} ft target.</p><div className="coverage-grid"><div><span>Flights over target</span><b>{flightsOverTarget} / {launches.length}</b></div><div><span>Best over score</span><b>{bestScore} pts</b></div></div><small className="insight-note">Over points are whole feet above target. Flights at or below target receive 0 points.</small></> : <InsightEmpty text="Log a flight to calculate TARC over points." />}</article></section>
}
