...
import { AlertTriangle, ArrowDownToLine, ArrowUpRight, BarChart3, Check, ChevronDown, CloudSun, Database, Download, Gauge, LogOut, Menu, Pencil, Plus, Rocket, RotateCcw, Settings2, Sparkles, Sun, Target, Trash2, Wind, X, Activity, Upload } from 'lucide-react'
import { useTheme } from './useTheme'
import { CartesianGrid, ComposedChart, Line, ReferenceLine, ResponsiveContainer, Scatter, Tooltip, XAxis, YAxis } from 'recharts'
import { adjustedAltitude, adjustedRegression, descentRegression, linearRegression, optimalRocketMass, predictAdjustedAltitude, predictDescentTime, totalMass, variableImpactConfidence, variableWeights, type DescentConditions, type WeatherReference } from './analytics'
...
import type { FlightRecord as Launch, EngineVersion, ExportEnvelope, Units, WorkspacePreferences } from './predictionTypes'
...
type WeatherVariable = keyof WeatherReference
type WeatherVariableSelection = Record<WeatherVariable, boolean>
const DEFAULT_WEATHER_VARIABLES: WeatherVariableSelection = { wind: true, pressure: true, humidity: true, temperature: true }
...
const createEmptyForm = (): FormValues => ({ date: todayLocal(), altitude: 800, flightTime: 0, descentTime: 0, parachuteSize: 18, rocketMass: 578, windSpeed: 4, airPressure: 29.92, humidity: 50, temperature: DEFAULT_TEMPERATURE, notes: '' })
...
const loadLocalPreferences = (): WorkspacePreferences => {
  try {
    ...
    if (raw === 'imperial' || raw === 'metric') return { ...DEFAULT_PREFERENCES, units: raw }
    const parsed = JSON.parse(raw ?? 'null') as Partial<WorkspacePreferences> | null
...
type Section = 'overview' | 'flights' | 'insights' | 'experiments' | 'settings'
const sectionForHash = (): Section => ({ dashboard: 'overview', flights: 'flights', analysis: 'insights', planner: 'experiments', settings: 'settings' }[window.location.hash.slice(1)] as Section | undefined) ?? 'overview'
...
const normalizeLaunches = (value: unknown, strictImport = false): Launch[] => {
  ...
  return value.flatMap((item) => {
    ...
    const normalized = { ...canonical, observedTilt: ['straight', 'slight', 'strong'].includes(String(stored.observedTilt)) ? stored.observedTilt : null, rocketMass: rocketMass + (Number.isFinite(legacyMotorMass) ? legacyMotorMass : 0), temperature: Number.isFinite(storedTemperature) ? storedTemperature : extracted?.temperature ?? DEFAULT_TEMPERATURE, notes: extracted?.notes ?? storedNotes } as unknown as Launch
    ...
    if (stored.observedTilt != null && !['straight', 'slight', 'strong'].includes(String(stored.observedTilt))) return []
...
const altitudePredictionEquation = (model: ReturnType<typeof adjustedRegression> | ReturnType<typeof linearRegression>, enabledVariables: WeatherVariableSelection = DEFAULT_WEATHER_VARIABLES) => {
  if (!model) return null
  const labels = model.coefficients.length === 1 ? ['mass'] : ['mass', 'wind', 'pressure', 'humidity', 'temperature']
  return `altitude = ${model.intercept.toFixed(2)}${model.coefficients.map((coefficient, index) => ({ coefficient, label: labels[index] })).filter(({ label }) => label === 'mass' || enabledVariables[label as WeatherVariable]).map(({ coefficient, label }) => equationTerm(coefficient, label)).join('')}`
}
...
function App() {
  ...
  const [units, setUnits] = useState<Units>(initialPreferences.units)
  const [targetAltitude, setTargetAltitude] = useState(initialPreferences.targetAltitude)
  const [plannerMinMass, setPlannerMinMass] = useState(initialPreferences.plannerMinMass)
  const [plannerMaxMass, setPlannerMaxMass] = useState(initialPreferences.plannerMaxMass)
  const [engineVersion, setEngineVersion] = useState<EngineVersion>(initialPreferences.engineVersion)
  ...
  const [simulatorVariables, setSimulatorVariables] = useState<WeatherVariableSelection>({ ...DEFAULT_WEATHER_VARIABLES })
  ...
  const [rocketMassInput, setRocketMassInput] = useState(String(createEmptyForm().rocketMass))
  const [showForm, setShowForm] = useState(false)
  const [editingLaunchId, setEditingLaunchId] = useState<string | null>(null)
  const [activeSection, setActiveSection] = useState<Section>(sectionForHash)
  ...
  useEffect(() => {
    const preferences: WorkspacePreferences = { units, targetAltitude, plannerMinMass, plannerMaxMass, engineVersion }
    try { localStorage.setItem(PREF_KEY, JSON.stringify(preferences)) } catch { /* storage may be unavailable */ }
  }, [units, targetAltitude, plannerMinMass, plannerMaxMass, engineVersion])

  useEffect(() => {
    const onPopState = () => setActiveSection(sectionForHash())
  ...
  useEffect(() => {
    ...
    fetchWorkspace(supabase, session.user.id).then((workspace) => {
      ...
      if (workspace.preferences) {
        ...
        setPlannerMinMass(workspace.preferences.plannerMinMass)
        setPlannerMaxMass(workspace.preferences.plannerMaxMass)
        setEngineVersion(workspace.preferences.engineVersion)
      }
    ...
    const channel = supabase.channel(`workspace-${session.user.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'launches', filter: `user_id=eq.${session.user.id}` }, (payload) => {
        ...
        if (payload.eventType === 'DELETE') {
          ...
          setLaunches((current) => { const next = current.filter((launch) => launch.id !== row.launch_id); return [...next, { id: row.launch_id, date: row.date, altitude: row.altitude, flightTime: row.flight_time, descentTime: row.descent_time, parachuteSize: row.parachute_size, rocketMass: row.rocket_mass, windSpeed: row.wind_speed, airPressure: row.air_pressure, humidity: row.humidity, temperature: row.temperature, notes: row.notes ?? '', observedTilt: row.observed_tilt ?? null, createdAt: row.created_at, updatedAt: row.updated_at }].sort((a, b) => a.date.localeCompare(b.date) || (a.createdAt && b.createdAt ? a.createdAt.localeCompare(b.createdAt) : 0)) })
          setVersions((current) => ({ ...current, [row.launch_id]: row.version }))
        }
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'user_preferences', filter: `user_id=eq.${session.user.id}` }, (payload) => {
        if (payload.eventType === 'DELETE') return
        const preference = payload.new as { units?: Units; target_altitude?: number; planner_min_mass?: number; planner_max_mass?: number; engine_version?: EngineVersion }
        if (preference.units) setUnits(preference.units)
        if (Number.isFinite(preference.target_altitude)) setTargetAltitude(preference.target_altitude!)
        if (Number.isFinite(preference.planner_min_mass)) setPlannerMinMass(preference.planner_min_mass!)
        if (Number.isFinite(preference.planner_max_mass)) setPlannerMaxMass(preference.planner_max_mass!)
        if (preference.engine_version) setEngineVersion(preference.engine_version)
      }).subscribe()
  ...
  const simulatorLaunches = useMemo(() => graphLaunches.map((launch) => ({
    ...launch,
    windSpeed: simulatorVariables.wind ? launch.windSpeed : 0,
    airPressure: simulatorVariables.pressure ? launch.airPressure : 0,
    humidity: simulatorVariables.humidity ? launch.humidity : 0,
    temperature: simulatorVariables.temperature ? launch.temperature : 0,
  })), [graphLaunches, simulatorVariables])
  const simulatorModel = useMemo(() => adjustedRegression(simulatorLaunches), [simulatorLaunches])
  const activeSimulatorWeather = useMemo(() => ({
    wind: simulatorVariables.wind ? simulatorWeather.wind : 0,
    pressure: simulatorVariables.pressure ? simulatorWeather.pressure : 0,
    humidity: simulatorVariables.humidity ? simulatorWeather.humidity : 0,
    temperature: simulatorVariables.temperature ? simulatorWeather.temperature : 0,
  }), [simulatorVariables, simulatorWeather])
  const descentModel = useMemo(() => descentRegression(graphLaunches), [graphLaunches])
  const simulatorConfidence = useMemo(() => variableImpactConfidence(simulatorModel, simulatorLaunches.map((launch) => [totalMass(launch), launch.windSpeed, launch.airPressure, launch.humidity, launch.temperature])), [simulatorModel, simulatorLaunches])
  const descentConfidence = useMemo(() => variableImpactConfidence(descentModel, [], true), [descentModel])
  const simulatorWeights = useMemo(() => variableWeights(simulatorModel, simulatorLaunches.map((launch) => [totalMass(launch), launch.windSpeed, launch.airPressure, launch.humidity, launch.temperature])), [simulatorModel, simulatorLaunches])
  ...
  const adjustedRecommendation = optimalRocketMass(simulatorModel, targetAltitude, activeSimulatorWeather)
  const simulatorMass = simulatorModel
    ? optimalRocketMass(simulatorModel, targetAltitude, activeSimulatorWeather)
    : rawModel && Math.abs(rawModel.coefficients[0]) > 1e-8
      ? (targetAltitude - rawModel.intercept) / rawModel.coefficients[0]
      : null
  const simulatorPrediction = simulatorModel && simulatorMass !== null ? predictAdjustedAltitude(simulatorModel, simulatorMass, activeSimulatorWeather) : rawModel && simulatorMass !== null ? rawModel.intercept + rawModel.coefficients[0] * simulatorMass : null
  ...
  const adjustedChart = useMemo(() => graphLaunches.map((launch, index) => ({ ...launch, mass: totalMass(launch), adjusted: simulatorModel ? adjustedAltitude(simulatorLaunches[index], simulatorModel, activeSimulatorWeather) : launch.altitude, fitted: simulatorModel ? simulatorModel.intercept + simulatorModel.coefficients[0] * totalMass(launch) + simulatorModel.coefficients[1] * activeSimulatorWeather.wind + simulatorModel.coefficients[2] * activeSimulatorWeather.pressure + simulatorModel.coefficients[3] * activeSimulatorWeather.humidity + simulatorModel.coefficients[4] * activeSimulatorWeather.temperature : 0 })).sort((a, b) => a.mass - b.mass), [graphLaunches, simulatorLaunches, simulatorModel, activeSimulatorWeather])
  ...
  const closeModal = () => { const next = createEmptyForm(); setForm(next); setRocketMassInput(String(next.rocketMass)); setEditingLaunchId(null); setShowForm(false) }
  const onSaveLaunch = async (event: React.FormEvent) => {
    event.preventDefault()
    const launch: Launch = { ...form, id: editingLaunchId ?? crypto.randomUUID(), createdAt: editingLaunchId ? launches.find(f => f.id === editingLaunchId)?.createdAt : new Date().toISOString() }
  ...
  const openNewLaunch = () => {
    ...
    setForm(next)
    setRocketMassInput(String(next.rocketMass))
  ...
  const persistPreferences = (next: Partial<WorkspacePreferences>) => {
    const preferences = { units, targetAltitude, plannerMinMass, plannerMaxMass, engineVersion, ...next }
    if (session && supabase && preferencesReady) void savePreferences(supabase, session.user.id, preferences).catch((error: Error) => setToast(`Preference sync failed · ${error.message}`))
  }
  ...
  const navigate = (section: Section) => {
    ...
    setActiveSection(section); setMobileNav(false)
  }

  const exportData = () => {
    const payload: ExportEnvelope = { schemaVersion: 2, exportedAt: new Date().toISOString(), engineVersion, preferences: { units, targetAltitude, plannerMinMass, plannerMaxMass, engineVersion }, flights: launches }
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'apexFlite-workspace-v2.json'; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 1000)
  ...
  const useCloudDataOnly = () => {
    if (session) localStorage.setItem(`apexFlite-migrated-${session.user.id}`, 'true')
    setPendingImport(null)
    setMigrationDismissed(true)
    setToast('Using cloud data · local backup left unchanged')
  }
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <header className="topbar"><button className="icon-button mobile-menu" onClick={() => setMobileNav(true)} aria-label="Open navigation"><Menu size={20} /></button><div className="breadcrumbs"><span>Primary rocket</span><span>/</span><b>{activeSection === 'overview' ? 'Dashboard' : activeSection === 'flights' ? 'Flights' : activeSection === 'insights' ? 'Analysis' : activeSection === 'experiments' ? 'Launch Planner' : 'Settings'}</b></div><div className="top-actions">{engineVersion === 'legacy-v1' && <button className="legacy-status-badge" onClick={() => navigate('settings')}><RotateCcw size={14} /> Legacy algorithms active</button>}<button className="unit-select" onClick={() => changeUnits(units === 'imperial' ? 'metric' : 'imperial')}><span className="unit-dot" /> {units === 'imperial' ? 'Imperial' : 'Metric'} <ChevronDown size={14} /></button><span className="sync-status-compact" title={syncStatus}><span className="sync-dot" />{session ? 'Cloud' : 'Local'}</span>{session ? <button className="avatar small" onClick={signOut} aria-label="Sign out">{session.user.email?.slice(0, 2).toUpperCase() ?? 'RT'}</button> : <span className="local-badge">GUEST</span>}</div></header>
      ...
      {showForm && <LaunchModal message={toast} form={form} setForm={setForm} onClose={() => { setShowForm(false); setEditingLaunchId(null) }} onSubmit={onSaveLaunch} units={units} updateForm={updateForm} editing={Boolean(editingLaunchId)} rocketMassInput={rocketMassInput} setRocketMassInput={setRocketMassInput} />}
      {pendingImport && !migrationDismissed && <MigrationDialog count={pendingImport.length} busy={cloudLoading} onImport={async () => { setCloudLoading(true); await importLocalData(); setCloudLoading(false) }} onDismiss={useCloudDataOnly} />}
      {toast && !showForm && <div className="toast" role="status"><Check size={16} /> {toast}</div>}
    </div>
  )
}
...
function MigrationDialog({ count, busy, onImport, onDismiss }: { count: number; busy: boolean; onImport: () => void | Promise<void>; onDismiss: () => void }) {
  return <div className="modal-backdrop migration-backdrop" role="dialog" aria-modal="true" aria-labelledby="migration-title"><div className="migration-dialog"><div className="migration-icon"><ArrowUpRight size={20} /></div><p className="eyebrow">FIRST CLOUD SIGN-IN</p><h2 id="migration-title">Choose your cloud data</h2><p>We found <strong>{count} {count === 1 ? 'flight' : 'flights'}</strong> saved in this browser. Merge them into your cloud workspace or continue with cloud data only.</p><ul><li>Merge adds only flight IDs not already in the cloud.</li><li>Your local backup remains unchanged either way.</li></ul><div className="migration-actions"><button className="secondary-button" onClick={onDismiss} disabled={busy}>Use cloud data</button><button className="primary-button" onClick={onImport} disabled={busy}>{busy ? 'Merging…' : 'Merge local flights'}</button></div></div></div>
}
...
function ImpactField({ label, confidence, weight, enabled, onEnabledChange, children }: { label: string; confidence?: number; weight?: number; enabled?: boolean; onEnabledChange?: (enabled: boolean) => void; children: React.ReactNode }) {
  const excluded = enabled === false
  return <div className={`impact-field${excluded ? ' is-excluded' : ''}`}><div className="impact-field-heading"><span>{label}</span>{onEnabledChange && <button type="button" role="switch" aria-checked={!excluded} aria-label={`Use ${label.toLowerCase()} in mass simulator`} onClick={() => onEnabledChange(excluded)}><i />{excluded ? 'OFF' : 'ON'}</button>}</div>{children}<small>{excluded ? 'Excluded from simulation' : confidence === undefined ? 'Impact confidence: learning' : `Impact confidence: ${confidence}%`}</small><small className="variable-weight">{excluded ? 'Variable weight: excluded' : weight === undefined ? 'Variable weight: learning' : `Variable weight: ${weight}%`}</small></div>
}

function AltitudePredictorWithConfidence({ units, model, fallbackModel, weather, setWeather, enabledVariables, setEnabledVariables, optimalMass, predictedAltitude, observedMasses, confidence, weights }: { units: Units; model: ReturnType<typeof adjustedRegression>; fallbackModel: ReturnType<typeof linearRegression>; targetAltitude: number; weather: WeatherReference; setWeather: React.Dispatch<React.SetStateAction<WeatherReference>>; enabledVariables: WeatherVariableSelection; setEnabledVariables: React.Dispatch<React.SetStateAction<WeatherVariableSelection>>; optimalMass: number | null; predictedAltitude: number | null; observedMasses: number[]; confidence: number[]; weights: number[] }) {
  const update = (field: keyof WeatherReference, value: number) => setWeather((current) => ({ ...current, [field]: value }))
  const setVariableEnabled = (field: WeatherVariable, enabled: boolean) => setEnabledVariables((current) => ({ ...current, [field]: enabled }))
...
function SettingsPanel({ units, setUnits, targetAltitude, setTargetAltitude, theme, setTheme, engineVersion, setEngineVersion, plannerMinMass, plannerMaxMass, setPlannerLimits }: { units: Units; setUnits: (u: Units) => void; targetAltitude: number; setTargetAltitude: (n: number) => void; theme: 'light' | 'dark'; setTheme: (t: 'light' | 'dark') => void; engineVersion: EngineVersion; setEngineVersion: (version: EngineVersion) => void; plannerMinMass: number; plannerMaxMass: number; setPlannerLimits: (minimum: number, maximum: number) => void }) {
  const metric = units === 'metric'
  const displayedTarget = targetAltitude * (metric ? .3048 : 1)
  return <><section className="page-heading"><div><p className="eyebrow">WORKSPACE PREFERENCES</p><h1>Settings</h1><p className="subtitle">Tune your planner, display, and compatibility defaults.</p></div></section><section className="settings-grid">
    <div className="settings-card"><div className="settings-card-heading"><div className="settings-big-icon"><Target size={20} /></div><div><h2>Planner targets</h2><p>Set the altitude goal and safe mass search range.</p></div></div><label>DEFAULT TARGET <div className="input-with-unit"><input type="number" min="1" value={Number(displayedTarget.toFixed(2))} onChange={event => setTargetAltitude(Number(event.target.value) / (metric ? .3048 : 1))} /><span>{metric ? 'm' : 'ft'}</span></div></label><div className="settings-inline-fields"><label>MINIMUM MASS<div className="input-with-unit"><input type="number" min="1" max={plannerMaxMass - .1} step="0.1" value={plannerMinMass} onChange={event => { const value = event.target.valueAsNumber; if (value > 0 && value < plannerMaxMass) setPlannerLimits(value, plannerMaxMass) }} /><span>g</span></div></label><label>MAXIMUM MASS<div className="input-with-unit"><input type="number" min={plannerMinMass + .1} step="0.1" value={plannerMaxMass} onChange={event => { const value = event.target.valueAsNumber; if (value > plannerMinMass) setPlannerLimits(plannerMinMass, value) }} /><span>g</span></div></label></div><p className="field-help">V2 recommendations are also restricted to masses represented by your flight log.</p></div>
    <div className={`settings-card engine-settings ${engineVersion === 'legacy-v1' ? 'legacy-selected' : ''}`}><div className="settings-card-heading"><div className="settings-big-icon purple"><RotateCcw size={20} /></div><div><h2>Prediction engine</h2><p>Choose the current validated planner or the preserved original workflow.</p></div></div><div className="unit-options"><button className={engineVersion === 'current-v2' ? 'selected' : ''} onClick={() => setEngineVersion('current-v2')}><b>Current v2</b><span>Auto-selected, bounded altitude planner</span>{engineVersion === 'current-v2' && <Check size={16} />}</button><button className={engineVersion === 'legacy-v1' ? 'selected' : ''} onClick={() => setEngineVersion('legacy-v1')}><b>Legacy v1</b><span>Original algorithms and experiments</span>{engineVersion === 'legacy-v1' && <Check size={16} />}</button></div>{engineVersion === 'legacy-v1' && <p className="legacy-explanation"><AlertTriangle size={15} /> Legacy v1 preserves the original numerical behavior, including known limitations, for repeatability.</p>}</div>
    <div className="settings-card"><div className="settings-card-heading"><div className="settings-big-icon purple"><Settings2 size={20} /></div><div><h2>Display units</h2><p>Choose how measurements appear throughout the app.</p></div></div><div className="unit-options"><button className={units === 'imperial' ? 'selected' : ''} onClick={() => setUnits('imperial')}><b>Imperial</b><span>ft · g · mph · inHg</span>{units === 'imperial' && <Check size={16} />}</button><button className={units === 'metric' ? 'selected' : ''} onClick={() => setUnits('metric')}><b>Metric</b><span>m · g · km/h · hPa</span>{units === 'metric' && <Check size={16} />}</button></div><p className="field-help">Canonical saved measurements are not changed.</p></div>
    <div className="settings-card"><div className="settings-card-heading"><div className="settings-big-icon"><Sun size={20} /></div><div><h2>Theme</h2><p>This device’s appearance preference.</p></div></div><div className="unit-options"><button className={theme === 'light' ? 'selected' : ''} onClick={() => setTheme('light')}><b>Light</b><span>Bright and clean</span>{theme === 'light' && <Check size={16} />}</button><button className={theme === 'dark' ? 'selected' : ''} onClick={() => setTheme('dark')}><b>Dark</b><span>Easy on the eyes</span>{theme === 'dark' && <Check size={16} />}</button></div><p className="field-help">Theme remains local to this device.</p></div>
  </section></>
}

function LaunchModal({ message, form, setForm, onClose, onSubmit, units, updateForm, editing, rocketMassInput, setRocketMassInput }: { message: string; form: FormValues; setForm: React.Dispatch<React.SetStateAction<FormValues>>; onClose: () => void; onSubmit: (e: React.FormEvent) => void; units: Units; updateForm: (field: keyof FormValues, value: string) => void; editing: boolean; rocketMassInput: string; setRocketMassInput: (value: string) => void }) {
  ...
  useEffect(() => {
    ...
    dialog.querySelector<HTMLButtonElement>('[aria-label="Close"]')?.focus()
    const viewport = window.visualViewport
    const resize = () => { if (viewport) { dialog.parentElement!.style.height = viewport.height + 'px'; dialog.parentElement!.style.top = viewport.offsetTop + 'px' } }
    ...
    return () => { document.body.style.overflow = overflow; previous?.focus(); dialog.removeEventListener('keydown', keydown); viewport?.removeEventListener('resize', resize); viewport?.removeEventListener('scroll', resize) }
  }, [])
  const field = (name: keyof FormValues, label: string, unit: string, step = '1', inputValue?: string, onInputChange?: (value: string) => void) => <label className="form-field">{label}<div className="input-with-unit"><input required={name !== 'notes'} type={name === 'date' ? 'date' : name === 'notes' ? 'text' : 'number'} step={step} value={inputValue ?? (typeof form[name] === 'number' ? Number(((form[name] as number) * (units === 'metric' ? ({ altitude: .3048, parachuteSize: 2.54, windSpeed: 1.60934, airPressure: 33.8639 } as Record<string, number>)[name] ?? 1 : 1)).toFixed(3)) : form[name] ?? '')} onChange={(event) => name === 'date' || name === 'notes' ? setForm((current) => ({ ...current, [name]: event.target.value })) : onInputChange ? onInputChange(event.target.value) : updateForm(name, String(Number(event.target.value) / (units === 'metric' ? ({ altitude: .3048, parachuteSize: 2.54, windSpeed: 1.60934, airPressure: 33.8639 } as Record<string, number>)[name] ?? 1 : 1)))} /><span>{unit}</span></div></label>; const temperatureInputValue = units === 'imperial' ? String(form.temperature) : String(fahrenheitToCelsius(form.temperature)); const updateTemperature = (value: string) => updateForm('temperature', value === '' ? '' : String(units === 'imperial' ? Number(value) : celsiusToFahrenheit(Number(value)))); return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="modal flight-modal" role="dialog" aria-modal="true" aria-labelledby="flight-modal-title" ref={dialogRef}><div className="modal-heading"><div><p className="eyebrow">{editing ? 'EDIT FLIGHT RECORD' : 'NEW FLIGHT RECORD'}</p><h2 id="flight-modal-title">{editing ? 'Edit flight' : 'Log a flight'}</h2><p>Capture the conditions while they are still fresh.</p></div><button className="icon-button" onClick={onClose} aria-label="Close"><X size={19} /></button></div><form onSubmit={onSubmit}><div className="modal-fields"><div className="form-section"><h3>Flight performance</h3><div className="form-grid">{field('date', 'Launch date', '', '1')}{field('altitude', 'Peak altitude', units === 'imperial' ? 'ft' : 'm')}{field('flightTime', 'Total flight time', 'sec', '0.1')}{field('descentTime', 'Descent time', 'sec', '0.1')}</div></div><div className="form-section"><h3>Configuration</h3><div className="form-grid">{field('rocketMass', 'Rocket mass (including motor)', 'g', 'any', rocketMassInput, (value) => { setRocketMassInput(value); updateForm('rocketMass', value) })}{field('parachuteSize', 'Parachute size', units === 'imperial' ? 'in' : 'cm')}{field('windSpeed', 'Wind speed', units === 'imperial' ? 'mph' : 'km/h', '0.1')}<label className="form-field">Observed flight path (optional)<select value={form.observedTilt ?? ''} onChange={event => setForm(current => ({ ...current, observedTilt: (event.target.value || null) as Launch['observedTilt'] }))}><option value="">Not recorded / unsure</option><option value="straight">Straight</option><option value="slight">Slight tilt</option><option value="strong">Noticeable tilt</option></select></label><p className="field-help full-field">Your visual observation is enough. No angle measurement needed; this is context, not a numerical altitude correction.</p></div></div><div className="form-section"><h3>Atmospheric conditions</h3><div className="form-grid">{field('airPressure', 'Air pressure', units === 'imperial' ? 'inHg' : 'hPa', '0.01')}{field('humidity', 'Humidity', '%', '1')}{field('temperature', 'Temperature', units === 'imperial' ? '°F' : '°C', '0.1', temperatureInputValue, updateTemperature)}<label className="form-field full-field">Notes <input type="text" placeholder="Anything worth remembering?" value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} /></label></div></div></div>{message && <p className="modal-feedback" role="status">{message}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button type="submit" className="primary-button"><Check size={16} /> {editing ? 'Update flight' : 'Save flight'}</button></div></form></div></div> }
