...
function FlightTable({ launches, targetAltitude, displayAltitude, displayMass, displayWind, displayTemperature, onDelete, onEdit }: { launches: Launch[]; targetAltitude: number; displayAltitude: (v: number) => string; displayMass: (v: number) => string; displayWind: (v: number) => string; displayTemperature: (v: number) => string; onDelete: (id: string) => void; onEdit: (launch: Launch) => void }) { const TARGET_DESCENT_TIME = 45; return <div className="table-card"><div className="flight-table table-header"><span>DATE</span><span>ALTITUDE</span><span>TARC SCORE</span><span>TOTAL MASS</span><span>WIND</span><span>TEMP</span><span>DESCENT</span><span>STATUS</span><span /></div>{launches.map((launch) => { const score = tarcScore(launch.altitude, launch.descentTime, targetAltitude, TARGET_DESCENT_TIME); const near = Math.abs(launch.altitude - targetAltitude) < 20; return <div className="flight-table" key={launch.id}><span className="date-cell"><b>{new Date(`${launch.date}T12:00:00`).toLocaleDateString('en-US', { month: 'short', day: '2-digit' })}</b><small>{launch.id.replace('flight-', 'Flight ')}</small></span><strong>{displayAltitude(launch.altitude)}</strong><span className={score ? 'tarc-score' : 'tarc-score zero'} title={`TARC score: ${score} pts (altitude + 4× descent deviation)`}>{score} pts</span><span>{displayMass(totalMass(launch))}</span><span className="wind-cell"><Wind size={14} /> {displayWind(launch.windSpeed)}</span><span>{displayTemperature(launch.temperature)}</span><span>{formatNumber(launch.descentTime, 1)} sec</span><span className={`status ${near ? 'on-target' : 'review'}`}><i /> {near ? 'On target' : 'Review'}</span><span className="row-actions"><button className="edit-button" onClick={() => onEdit(launch)} aria-label={`Edit ${launch.id}`}><Pencil size={14} /></button><button className="delete-button" onClick={() => onDelete(launch.id)} aria-label={`Delete ${launch.id}`}><Trash2 size={15} /></button></span></div>})}</div> }
