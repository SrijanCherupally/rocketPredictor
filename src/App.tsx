  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        <section className="graph-filters"><label>Mass range: {formatMass(selectedMassWindow[0])}–{formatMass(selectedMassWindow[1])} g<input type="range" min={graphBounds.minMass} max={graphBounds.maxMass} value={selectedMassWindow[0]} onChange={(event) => setMassWindow([Number(event.target.value), selectedMassWindow[1]])} /><input type="range" min={graphBounds.minMass} max={graphBounds.maxMass} value={selectedMassWindow[1]} onChange={(event) => setMassWindow([selectedMassWindow[0], Number(event.target.value)])} /></label><label>Launch dates: {new Date(selectedDateWindow[0]).toLocaleDateString()}–{new Date(selectedDateWindow[1]).toLocaleDateString()}<input type="range" min={graphBounds.minDate} max={graphBounds.maxDate} value={selectedDateWindow[0]} onChange={(event) => setDateWindow([Number(event.target.value), selectedDateWindow[1]])} /><input type="range" min={graphBounds.minDate} max={graphBounds.maxDate} value={selectedDateWindow[1]} onChange={(event) => setDateWindow([selectedDateWindow[0], Number(event.target.value)])} /></label><button className="text-button" onClick={() => { setMassWindow(null); setDateWindow(null) }}>Reset filters</button></section>
