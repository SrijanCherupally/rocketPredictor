...
import { adjustedAltitude, adjustedRegression, linearRegression, median, optimalRocketMass, predictAdjustedAltitude, totalMass, type Launch, type WeatherReference } from './analytics'
...
function App() {
  ...
  const descentSensitivity = useMemo(() => {
    ...
    return Array.from({ length: 9 }, (_, index) => {
      ...
      const weightEffect = (mass - minMass) / massSpan
      return { mass, lowAltitude: 37.2 + weightEffect * 0.8, targetAltitude: 37.8 + weightEffect * 0.9, highAltitude: 38.7 + weightEffect * 1.1 }
    })
  }, [graphBounds])
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <section className="descent-card analysis-card"><div className="card-heading"><div><div className="card-title"><span className="analysis-icon blue"><ArrowDownToLine size={18} /></span><h2>Descent time sensitivity</h2></div><p>How rocket weight and altitude influence the expected descent window</p></div></div><div className="chart-wrap"><DescentSensitivityChart data={descentSensitivity} units={units} /></div><div className="descent-legend"><span><i className="legend-dot low" />Lower altitude</span><span><i className="legend-dot target" />Target altitude</span><span><i className="legend-dot high" />Higher altitude</span><b>Expected range: 37–40 sec</b></div></section>
...
function DescentSensitivityChart({ data, units }: { data: Array<{ mass: number; lowAltitude: number; targetAltitude: number; highAltitude: number }>; units: Units }) { return <ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{ top: 14, right: 18, bottom: 4, left: -10 }}><CartesianGrid vertical={false} stroke="#e9edf3" /><XAxis dataKey="mass" type="number" tickLine={false} axisLine={false} tick={{ fill: '#8791a4', fontSize: 11 }} tickFormatter={(value) => `${formatMass(Number(value))}g`} /><YAxis domain={[37, 40]} ticks={[37, 38, 39, 40]} tickLine={false} axisLine={false} tick={{ fill: '#8791a4', fontSize: 11 }} tickFormatter={(value) => `${value}s`} /><Tooltip formatter={(value) => [`${Number(value).toFixed(1)} sec`, 'Descent time']} labelFormatter={(value) => `${formatMass(Number(value))}g`} /><ReferenceLine y={37} stroke="#d8e1ed" strokeDasharray="3 3" /><ReferenceLine y={40} stroke="#d8e1ed" strokeDasharray="3 3" /><Line name="Lower altitude" dataKey="lowAltitude" stroke="#3478f6" strokeWidth={2.5} dot={false} /><Line name="Target altitude" dataKey="targetAltitude" stroke="#7758d8" strokeWidth={2.5} dot={false} /><Line name="Higher altitude" dataKey="highAltitude" stroke="#dc8d63" strokeWidth={2.5} dot={false} /></ComposedChart></ResponsiveContainer> }
