...
import { adjustedAltitude, adjustedRegression, descentRegression, linearRegression, median, optimalRocketMass, predictAdjustedAltitude, predictDescentTime, totalMass, variableImpactConfidence, type DescentConditions, type Launch, type WeatherReference } from './analytics'
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <AltitudePredictorWithConfidence units={units} model={adjustedModel} fallbackModel={rawModel} targetAltitude={targetAltitude} weather={simulatorWeather} setWeather={setSimulatorWeather} optimalMass={simulatorMass} predictedAltitude={simulatorPrediction} observedMasses={graphLaunches.map(totalMass)} confidence={altitudeConfidence} />
        <DescentPredictorWithConfidence units={units} model={descentModel} conditions={descentConditions} setConditions={setDescentConditions} predictedDescent={predictedDescent} confidence={descentConfidence} />
...
function ImpactField({ label, confidence, children }: { label: string; confidence?: number; children: React.ReactNode }) {
  return <label className="impact-field"><span>{label}</span>{children}<small>{confidence === undefined ? 'Impact confidence: learning' : `Impact confidence: ${confidence}%`}</small></label>
}

function AltitudePredictorWithConfidence({ units, model, fallbackModel, targetAltitude, weather, setWeather, optimalMass, predictedAltitude, observedMasses, confidence }: { units: Units; model: ReturnType<typeof adjustedRegression>; fallbackModel: ReturnType<typeof linearRegression>; targetAltitude: number; weather: WeatherReference; setWeather: React.Dispatch<React.SetStateAction<WeatherReference>>; optimalMass: number | null; predictedAltitude: number | null; observedMasses: number[]; confidence: number[] }) {
  ...
  return <section className="mass-simulator"><div className="simulator-heading"><div><p className="eyebrow">FLIGHT CONFIGURATION</p><h2>Rocket mass simulator</h2><p>Find the mass that should reach your target altitude in the conditions you enter.</p></div><div className="simulator-badge">{model ? 'WEATHER MODEL' : fallbackModel ? 'RAW MODEL' : 'NEEDS 3 FLIGHTS'}</div></div><div className="simulator-body"><div className="simulator-inputs">{fields.map((input) => <ImpactField key={input.field} label={input.label} confidence={input.confidence}><div className="simulator-input"><input type="number" min={input.min} step={input.step} value={input.value} onChange={(event) => update(input.field, input.convert(Number(event.target.value)))} /><span>{input.unit}</span></div></ImpactField>)}</div><div className="simulator-result"><span className="footer-label">OPTIMAL ROCKET MASS</span><strong>{optimalMass !== null && optimalMass > 0 ? `${formatRecommendationMass(optimalMass)} g` : 'Collect more data'}</strong><span>{predictedAltitude !== null ? `Predicted ${formatNumber(predictedAltitude)} ft at target conditions` : model || fallbackModel ? 'Model needs a meaningful mass trend' : 'Log at least 3 flights to simulate'}</span>{model && <small>Mass impact confidence: {confidence[0]}%</small>}{outsideRange && <small>Recommendation is outside your logged mass range ({formatMass(minObserved)}–{formatMass(maxObserved)} g).</small>}</div></div></section>
}

function DescentPredictorWithConfidence({ units, model, conditions, setConditions, predictedDescent, confidence }: { units: Units; model: ReturnType<typeof descentRegression>; conditions: DescentConditions; setConditions: React.Dispatch<React.SetStateAction<DescentConditions>>; predictedDescent: number | null; confidence: number[] }) {
  const update = (field: keyof DescentConditions, value: number) => setConditions((current) => ({ ...current, [field]: value }))
  const fields: Array<{ label: string; field: keyof DescentConditions; value: number; unit: string; confidence?: number; min?: number; step?: number; convert?: (value: number) => number }> = [
