...
function LabResults({ suites, conditions, setConditions, targetAltitude, units }: Omit<Props, 'launches'> & { suites: Suites }) {
  ...
  return <>
    ...
    <div className="lab-section-heading"><div><h3>Descent sensitivity</h3><p>Change mass while holding the entered altitude, parachute and weather constant.</p></div><ModelSelect task="descent" label="Descent algorithm" results={suites.descent.results} value={descentMethod} setValue={setDescentMethod} /></div>
    ...
    <div className="lab-section-heading"><div><h3>Experimental rocket mass prediction</h3><p>Target: {number(targetAltitude * (metric ? .3048 : 1), metric ? ' m' : ' ft')} · shares weather with the Overview prediction tools.</p></div><ModelSelect task="altitude" label="Mass algorithm" results={suites.altitude.results} value={massMethod} setValue={setMassMethod} /></div>
    <div className="lab-summary lab-mass-result" aria-live="polite"><div><span>Mass for target altitude</span><strong>{number(solution.mass, ' g')}</strong><small>{solution.reason}</small></div><div><span>Held-out mass error</span><strong>{number(altitude.massMae, ' g')}</strong><small>{altitude.massCount}/{altitude.total} flights had a supported solution</small></div><div><span>Held-out altitude error</span><strong>{number(altitude.mae === null ? null : altitude.mae * (metric ? .3048 : 1), metric ? ' m' : ' ft')}</strong><small>{altitude.tested}/{altitude.total} flights tested</small></div></div>
    <div className="lab-weather-effect" aria-live="polite"><b>{usesWeather(massMethod, 'altitude') ? 'Weather-aware mass model' : 'Mass-only model · weather is not an input'}</b><p>{usesWeather(massMethod, 'altitude') ? weatherEffect.delta === null ? 'Weather effect cannot be compared: the current or median-weather scenario has no supported mass solution.' : `${weatherEffect.delta >= 0 ? '+' : ''}${weatherEffect.delta.toFixed(2)} g versus ${number(weatherEffect.referenceMass, ' g')} at median logged weather. A zero change can mean the log has not learned a weather effect.` : 'This model uses mass only, so weather edits do not change its recommendation. Select Original weather regression, Regularized regression, Nearest flights, or Neural network to include weather.'}</p><small>Editing scenario weather changes predictions, not validation scores; each validation flight uses its own recorded weather.</small></div>
