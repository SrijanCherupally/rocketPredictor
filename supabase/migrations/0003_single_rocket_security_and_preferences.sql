...
create policy "Owners read rocket preferences" on public.rocket_preferences
  for select to authenticated
  using (exists (
    select 1 from public.rockets
    where rockets.id = rocket_preferences.rocket_id
      and rockets.user_id = auth.uid()
  ));

create policy "Owners create rocket preferences" on public.rocket_preferences
...
create policy "Owners update rocket preferences" on public.rocket_preferences
...
create policy "Owners delete rocket preferences" on public.rocket_preferences
