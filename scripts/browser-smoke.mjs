...
try {
  ...
  const validationTable = lab.locator('.lab-comparison').nth(1).locator('table').first()
  ...
  await lab.getByText('Mass-only model · weather is not an input', { exact: true }).waitFor()
  ...
  await lab.locator('.lab-comparison').nth(1).getByRole('button', { name: 'Export validation details' }).click()
  ...
  await lab.locator('.lab-comparison').nth(1).getByRole('button', { name: 'Original weather regression', exact: true }).click()
  await lab.locator('.lab-comparison').nth(1).locator('summary').click()
  ...
  if (process.env.TEST_REPORTED_FILE) {
    ...
    await lab.locator('.lab-comparison').nth(1).getByText('1752.27', { exact: true }).waitFor()
    await lab.getByLabel('Descent algorithm').selectOption('neural')
    await lab.getByLabel('Mass algorithm').selectOption('neural')
