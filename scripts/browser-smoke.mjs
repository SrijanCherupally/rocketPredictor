...
try {
  ...
  await page.evaluate(() => localStorage.setItem('apexflite-prefs-v1', JSON.stringify({ units: 'imperial', targetAltitude: 800, plannerMinMass: 500, plannerMaxMass: 700, engineVersion: 'current-v2' })))
  await page.reload()
  ...
  await page.getByRole('heading', { name: 'Weather-aware model' }).waitFor()
  ...
  const pressureToggle = simulator.getByRole('switch', { name: 'Use air pressure in mass simulator' })
  await pressureToggle.click()
  assert.equal(await pressureToggle.getAttribute('aria-checked'), 'false')
  assert.equal(await simulator.getByRole('spinbutton', { name: 'AIR PRESSURE', exact: true }).isDisabled(), true)
  assert.doesNotMatch(await simulator.locator('.prediction-equation').innerText(), /pressure/)
  assert.equal(await overviewMass.innerText(), await simulator.locator('.simulator-result strong').innerText())
  await pressureToggle.click()
  assert.equal(await pressureToggle.getAttribute('aria-checked'), 'true')
