...
try {
  for (const name of ['analytics', 'experiments', 'massRange', 'seed', 'predictionV2', 'predictionTypes', 'legacyEngine', 'dayRecommendation', 'flightInsights', 'analysisEngine', 'cloud']) {
  ...
  if (process.argv.includes('--benchmark')) {
    ...
    const tests = ['experiments.test.mjs', 'prediction-v2.test.mjs', 'flight-analysis.test.mjs']
    for (const name of tests) writeFileSync(join(folder, name), readFileSync(new URL(`../tests/${name}`, import.meta.url), 'utf8'))
    const result = spawnSync(process.execPath, ['--test', ...tests.map(name => join(folder, name))], { stdio: 'inherit' })
